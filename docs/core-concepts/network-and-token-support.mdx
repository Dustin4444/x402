...
|-----------|------------------|-----------------|
| **EVM** | Any ERC-20 token | EIP-3009, Permit2, or ERC-7710 |
...
x402 supports three asset transfer methods on EVM:

| Method | When Used | How It Works |
|--------|-----------|--------------|
| **EIP-3009** | Tokens with `transferWithAuthorization` (e.g., USDC) | Single off-chain signature, simplest flow. Requires `extra.name` and `extra.version`. |
| **Permit2** | Any ERC-20 token | Uses Uniswap's [Permit2](https://docs.uniswap.org/contracts/v4/deployments) contract + `x402ExactPermit2Proxy`. `extra.name` and `extra.version` required only for EIP-2612 tokens. |
| **ERC-7710** | Smart accounts with delegation support | Uses ERC-7710 delegation for smart contract accounts. `extra.name` and `extra.version` are optional. |

All methods provide gasless transfers (facilitator sponsors gas) and signature-based authorization. EIP-3009 is preferred when available (simpler, no approval step). Permit2 serves as the universal fallback for any ERC-20 token. ERC-7710 is the option for smart contract accounts.
...
* Any ERC-20 token works on any EVM facilitator (via EIP-3009, Permit2, or ERC-7710)
